...
def set_student(data_list):
    """ Записывает данные ученика в словарь. Пример записи: {'Иванов': (['Иван', '1A'], {Инф:[5,5,5,5,4]})} """
...
def set_rating(last_name, lesson, rating):
    ...
    if student_db.get(last_name) is None:
        ...
        if lesson in student_db[last_name][1]:
            student_db[last_name][1][lesson].extended(rating)
        else:
...
def save_db():
    ...
    json.dump(student_db, open('HW8/db_student.json', 'w'))


def load_db():
    """ Сохранение данных учеников из файла формата .json в словарь 'student_db' """
    global student_db
    if os.stat('HW8/db_student.json').st_size == 0:
        student_db = {}
    else:
        student_db = json.load(open('HW8/db_student.json'))
